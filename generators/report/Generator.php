...
namespace backend\templates\generators\crudReport;
...
class Generator extends GeneratorDefault
{
    ...
    public $baseControllerClass = 'backend\components\ReportController';
    public $viewPath = '@backend\modules\statistics\views\<controller_name>';
    public $controllerClass = 'backend\modules\statistics\controllers\<controller_name>';
    public $searchModelClass = 'backend\modules\statistics\models\<class_name>Search';
    ...
    protected $_calculateColumns = [];
    protected $_dividendColumns = [];
    protected $_averageColumns = [];
    protected $_distinctColumns = [];
    protected $_dividerColumns = [];
    protected $_summaryColumns = [];

    public function getDividendColumns($id)
    {
        if (!isset($this->_dividendColumns[$id])) {
            return false;
        }
        return $this->_dividendColumns[$id];
    }

    public function getDividerColumns($id)
    {
        if (!isset($this->_dividerColumns[$id])) {
            return false;
        }
        return $this->_dividerColumns[$id];
    }

    public function getAverageColumns($id)
    {
        if (!isset($this->_averageColumns[$id])) {
            return false;
        }
        return $this->_averageColumns[$id];
    }

    public function getDistinctColumns($id)
    {
        if (!isset($this->_distinctColumns[$id])) {
            return false;
        }
        return $this->_distinctColumns[$id];
    }

    public function getSummaryColumns($id)
    {
        if (!isset($this->_summaryColumns[$id])) {
            return false;
        }
        return $this->_summaryColumns[$id];
    }
    ...
    public function getGridViewColumns()
    {
        return \yii\helpers\ArrayHelper::merge(
            $this->getPrimaryKeys(),
            $this->getCalculateColumns()
        );
    }

    protected $_columnTypes = [];

    public function getColumnTypes($id)
    {
        return $this->_columnTypes[$id];
    }

    public function getCalculateColumns()
    {
        if ($this->_calculateColumns) {
            return $this->_calculateColumns;
        }

        $aggregateColumns = array_diff_key(
            array_flip($this->getTableSchema()->columnNames),
            array_flip($this->getPrimaryKeys())
        );

        foreach ($this->getTableSchema()->columns as $k => $column) {
            if(array_key_exists($column->name, $aggregateColumns)) {
                if (preg_match('~(.*)_average$~', $column->name, $match)) {
                    $this->_averageColumns[$match[1]] = $column->name;
                    $this->_calculateColumns[] = $match[1];
                    $this->_columnTypes[$match[1]] = ['type' => 'float', 'scale' => $column->scale ?: 4];
                    continue;
                }
                if (preg_match('~(.*)_distinct$~', $column->name, $match)) {
                    $this->_distinctColumns[$match[1]] = $column->name;
                    $this->_calculateColumns[] = $match[1];
                    $this->_columnTypes[$match[1]] = ['type' => 'float', 'scale' => $column->scale ?: 4];
                    continue;
                }
                if (preg_match('~(.*)_dividend$~', $column->name, $match)) {
                    $this->_dividendColumns[$match[1]] = $column->name;
                    $this->_calculateColumns[] = $match[1];
                    $this->_columnTypes[$match[1]] = ['type' => 'float', 'scale' => $column->scale ?: 4];
                    continue;
                } 
                if (preg_match('~(.*)_divider$~', $column->name, $match)) {
                    $this->_dividerColumns[$match[1]] = $column->name;
                    $this->_calculateColumns[] = $match[1];
                    $this->_columnTypes[$match[1]] = ['type' => 'float', 'scale' => $column->scale ?: 4];
                    continue;
                }   
                switch($column->type) {
                    case 'float':
                    case 'decimal':
                        $this->_columnTypes[$column->name] = ['type' => 'float', 'scale' => $column->scale];
                        break;
                    case 'int':
                    case 'bigint':
                    default:
                        $this->_columnTypes[$column->name] = ['type' => 'int'];
                }

                $this->_summaryColumns[$column->name] = $column->name;
                $this->_calculateColumns[] = $column->name;
            }
        }

        $this->_calculateColumns = array_unique($this->_calculateColumns);
        return $this->_calculateColumns;
    }
    ...
    public function getOtherKeys() {
        ...
        foreach ($this->getColumnNames() as $column) {
            if (strpos($column, "pk_") === false) {
                if (($pos = mb_stripos($column, "tk_")) === false) {
                    continue;
                }
    ...
    {
        /* @var $modelClass string|\common\components\ActiveRecordReport */
     ...
     * @return string[]
     */
    public function getRequiredPrimaryKeys()
    {
        /* @var $modelClass string|\common\components\ActiveRecordReport */
        $modelClass = $this->modelClass;
        return array_intersect($modelClass::primaryKey(),
            ArrayHelper::merge(
                $modelClass::datetimeAttributes(),
                $modelClass::dateAttributes()
            )
        );
    }
    ...
    public function generateUrlPath() {
        return str_replace(array("@backend/modules", 'views/'), '', $this->viewPath);
    }
    ...
    public function generateActiveBooleanField($attribute)
    {
        ...
        return "\$form->field(\$model, '$attribute')->dropDownList(array(0 => Yii::t('info', 'Да'), 1 => Yii::t('info', 'Нет')), [])";
    }
    ...
    {
        return 'CRUD Report Generator (Veselov)';
    }
...
//                        $table
    ...
    public function getModelHasDates()
    {
        ...
        if (!isset($hasDates)) {
            return $hasDates = count(array_intersect(/*$this->datetimeAttributes*/ $this->__datetimeAttributes, $this->getColumnNames())) > 0;
        }
        return $hasDates;
    }
     ...
     * @return array|bool
     */
    public function isIdModelVirtual($attribute, $tableName = null)
    {
        $links = $this->getLinks($tableName);
        ...
        if (!empty($table)) {
            ...
            $modelClass = str_replace('\reports', '', "{$this->modelNS}\\$class");
    ...
    public function generateTimestampAttributes($tableSchema)
    {
        if (sizeof($this->__datetimeAttributes)<1) {
            foreach ($tableSchema->columns as $column) {
                if (in_array(strtolower($column->dbType), array('timestamp', 'date', 'datetime'))) {
    ...
    public function generateRelationCode($relationName, $relationModel, $attribute)
    {
        ...
        $tb = $cl::getTableSchema(); //(\yii\helpers\Inflector::underscore('AccountOperation'));
        ...
        $code = "if (!\$model->{$relationName}) { return null; } return \$model->{$relationName}->{$field_name};";
    ...
    public function getReplaceString($attribute) {
        return preg_replace('/^(pk|tk)_/', '', $attribute);
    }
    ...
    public function generateColumn($tableSchema, $column, $pull_right = false)
    {
        ...
        $strtr = $this->getReplaceString($attribute);

        if ($this->isIdModelVirtual($strtr, $this->tableName) !== false) {
            return "
            '$attribute' => [
                'class' => 'backend\\components\\grid\\".Inflector::id2camel(str_replace('pk_', '', $attribute), '_')."Column',
                'attribute' => '$attribute',
        ...
        if (in_array($attribute, $this->__datetimeAttributes)) {
            ...
            '$attribute' => [
                ...
                'hAlign' => 'center',
                'width' => '10%',
                'value' => function (\$model, \$key, \$index, \$widget) use (\$searchModel) {
                    \$value = \$model->$attribute;
                    return \$value ? Yii::\$app->formatter->asDatetime(strtotime(\$value), \$searchModel->getDateFormat('$attribute', 'php')) : null;
                },
            ],";
        }
        ...
        if (preg_match('~(.*)_amount|_sum$~', $attribute, $match)) {
            return "
            '$attribute' => [
                'class' => 'backend\\components\\grid\\CurrencyColumn',
                'pageSummary' => true,
    ...
    protected function generateKartikActiveField($attribute, $column = null)
    {
        ...
        'options' => ['placeholder' => " . $this->generateString("Select a {$relationModel} ...") . "],
        'pluginOptions' => [
            'allowClear' => true,
        ],
    ]);";
    ...
    public function generateSearchRules()
    {
        ...
        if (($table = $this->getTableSchema()) === false) {
            ...
            if (count($_columns) != count($columns)) {
                ...
                $rules[] = "[['" . implode("', '", $_columns) . "'], 'date', 'format' => 'MM/DD/YY HH:mm - MM/DD/YY HH:mm', 'message' => " .
                    $this->generateString('Недопустимый диапазон дат.') . "],";
            }

            if (in_array('pk_workshift_id', $this->getPrimaryKeys())) {
                $rules[] = "[['pk_created_at'], 'required', 'when' => function(\$model) {
               if (\$model->pk_workshift_id) {
                   return false;
               } else {
                   return true;
               }
                ...
                $rules[] = "[['pk_created_at'], 'required']";
            }

            if (in_array('pk_terminal_group_id', $this->getPrimaryKeys())) {
                $rules[] = "[['pk_workshift_id'], 'required', 'when' => function(\$model) {
                if (!\$model->pk_created_at) {
                    return true;
                } else {
                    return false;
                }
        ...
        foreach ($table->columns as $column) {
            if ($this->isIdModelVirtual($this->getReplaceString($column->name))) {
//            if ($this->isIdModelVirtual(str_replace(array('pk_','t_'), '', $column->name))) {
        ...
        foreach ($types as $type => $columns) {
            if ($type == 'datetime') {
                $rules[] = "[['" . implode("', '", $columns) . "'], 'date', 'format' => 'MM/DD/YY HH:mm - MM/DD/YY HH:mm', 'message' => " .
                    $this->generateString('Недопустимый диапазон дат.') . "]";
            } else {
...
//        $conditions[] = "\$this->query->orderBy(['".implode("' => SORT_ASC, '", $this->getPrimaryKeys())."' => SORT_ASC]);\n";
